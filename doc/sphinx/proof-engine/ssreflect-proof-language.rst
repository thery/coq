...
.. example::
  ...
  instead. For both cases it is ``(plus _ _)`` and matches the subterm
  ``plus (plus x y) z`` thus instantiating the latter ``_`` with ``z``. 
  ...
  results in an error, since ``y`` and ``z`` do no unify but the type
  of the eliminator requires the second argument of
  ``P`` to be the same as the second argument of ``plus`` in the
...
:ref:`views_and_reflection_ssr`.4), it is quite frequent to need to decompose the logical
...
|SSR| has a special syntax apply/ for combining simultaneous goal
interpretation operations and bookkeeping steps in a single tactic.

With the hypotheses of section :ref:`interpreting_assumptions_ssr`, the following script, where
...
proof of ``A`` or a proof of ``B``. 
The tactics ``left`` and ``right`` change the goal
``A \/ B`` to ``A`` and ``B``, respectively; dually, the tactic 
``case``
reduces the goal ``A \/ B => G`` to two subgoals
``A => G`` and
``B => G``.
...
.. _:ref:`rewriting_ssr`  Rewriting: :///home/steck/ssreflect.html#sec561
.. _Cover: :///home/steck/index.html
.. _Errors: :///home/steck/error-index.html
.. _Table of contents: :///home/steck/toc.html
.. _Community: :///community
.. _:ref:`views_and_reflection_ssr`  Views and reflection: :///home/steck/ssreflect.html#sec582
.. _8.3.2: :///home/steck/tactics.html#intros-pattern
.. _:ref:`basic_tactics_ssr`  Basic tactics: :///home/steck/ssreflect.html#sec535
